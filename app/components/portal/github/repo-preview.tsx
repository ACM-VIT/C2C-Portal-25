...
import { listCommitsAction, getContentsAction, saveInstallationAction, getRepoAction, listBranchesAction, listPullsAction, listIssuesAction, listReleasesAction, listLanguagesAction } from '@/app/actions/github';
    ...
    <div className="mt-8">
      <div className="bg-black/30 border border-white/10 rounded-2xl p-6">
        <div className="flex items-start justify-between gap-4">
          <div>
            <h2 className="text-2xl" style={{ fontFamily: "'Pilat Extended', Arial, sans-serif" }}>{owner}/{repo}</h2>
            {repoInfo && (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-6">
        ...
        <Card title="Branches">
          <ul className="space-y-2">
            {branches.map((b) => (
              <li key={b.name} className="p-2 border border-white/10 rounded bg-black/20 flex items-center justify-between">
                <span className="font-mono text-sm">{b.name}</span>
                {b.protected && <span className="text-[10px] px-2 py-0.5 rounded bg-white/10">protected</span>}
              </li>
            ))}
          ...
          <ul className="space-y-2">
            {pulls.map((p) => (
              <li key={p.id} className="p-3 border border-white/10 rounded bg-black/20">
                <div className="font-semibold">#{p.number} {p.title}</div>
                <div className="text-xs text-gray-400">by {p.user?.login} • {new Date(p.created_at || Date.now()).toLocaleString()}</div>
                <a className="text-xs underline" href={p.html_url} target="_blank" rel="noreferrer">View PR</a>
              </li>
            ))}
          ...
          <ul className="space-y-2">
            {issues.map((i) => (
              <li key={i.id} className="p-3 border border-white/10 rounded bg-black/20">
                <div className="font-semibold">#{i.number} {i.title}</div>
                <div className="text-xs text-gray-400">by {i.user?.login} • {new Date(i.created_at || Date.now()).toLocaleString()}</div>
                <a className="text-xs underline" href={i.html_url} target="_blank" rel="noreferrer">View Issue</a>
              </li>
            ))}
          ...
          <ul className="space-y-2">
            {releases.map((r) => (
              <li key={r.id} className="p-3 border border-white/10 rounded bg-black/20">
                ...
                <div className="text-xs text-gray-400">{r.prerelease ? 'Pre-release' : r.draft ? 'Draft' : 'Stable'} • {new Date(r.published_at || Date.now()).toLocaleDateString()}</div>
                <a className="text-xs underline" href={r.html_url} target="_blank" rel="noreferrer">View Release</a>
              </li>
            ))}
        ...
        <Card title={`Files ${currentPath ? `in /${currentPath}` : '(root)'}`}>
          <ul className="space-y-1">
            {contents.map((item: any) => (
              <li key={item.sha} className="flex items-center justify-between p-2 border border-white/10 rounded bg-black/20">
                <div className="flex items-center gap-2">
                  <span className="text-xs px-2 py-0.5 rounded bg-white/10">{item.type}</span>
                  <span>{item.name}</span>
                </div>
                {item.type === 'file' && (
                  <a href={item.html_url} target="_blank" rel="noreferrer" className="text-sm underline">View</a>
                )}
              </li>
            ))}
...
function Card({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <section className="bg-black/30 border border-white/10 rounded-2xl p-5">
      <h3 className="text-lg mb-3" style={{ fontFamily: "'Pilat Extended', Arial, sans-serif" }}>{title}</h3>
      {children}
    </section>
  );
}
